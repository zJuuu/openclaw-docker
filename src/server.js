...
class GatewayManager {
  ...
  async #doStart() {
    ...
    try {
      ...
      await cli.exec("config", "set", "gateway.controlUi.dangerouslyDisableDeviceAuth", "true");
...
app.post("/get-started/api/run", requireAuth, async (req, res) => {
  ...
  try {
...

    {
      ...
      await cli.exec("config", "set", "gateway.controlUi.allowInsecureAuth", "true");
      // Skip device-identity/pairing for Control UI since our proxy handles auth.
      await cli.exec("config", "set", "gateway.controlUi.dangerouslyDisableDeviceAuth", "true");
      // Allow the proxied origin for the Control UI.
      await cli.exec("config", "set", "gateway.controlUi.allowedOrigins", JSON.stringify([`http://127.0.0.1:${INTERNAL_GATEWAY_PORT}`]));
